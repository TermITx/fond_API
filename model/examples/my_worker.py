...
def main(lookback_window_length: int):
    days_to_subtract = 300
    date_start = (date.today()- timedelta(days=days_to_subtract)).strftime("%Y-%m-%d")
    ...
    for i in range(0, len(QUANDL_TICKERS), 1):
        ...
        for ticker in new_tickers:
            original_path = f'{os.path.join("model", CPD_QUANDL_OUTPUT_FOLDER(lookback_window_length), ticker + ".csv")}'
            tmp_path = f'{os.path.join("model", CPD_QUANDL_OUTPUT_FOLDER(lookback_window_length), ticker +  "tmp"+  ".csv")}'
